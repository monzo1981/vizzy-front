  ...
  const handleSend = async (message?: string) => {
    ...
    try {
      ...
      if (selectedImage && selectedImageFile) {
        ...
        response = await n8nWebhook.current.sendImageMessage(base64, messageToSend)
      } else {
        // Send text only
      ...
      <div className={`min-h-screen flex flex-col transition-all duration-300 ${isOpen ? 'lg:ml-20' : 'lg:ml-20'}`}>
        ...
        {!hasMessages ? (
          ...
          <main className="flex-1 flex flex-col items-center justify-center px-6 pb-8">
            <h1 className="text-[57px] font-medium text-[#11002E] text-center leading-none mb-16">
              {"What's on the agenda today?"}
            </h1>

            {/* Search Input */}
            <div className="w-full max-w-3xl sm:max-w-4xl mb-10">
              <div className="relative bg-white/60 backdrop-blur-xl rounded-[50px] border border-white/30 px-8 py-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <Plus 
                      size={32} 
                      className="text-[#4248FF] cursor-pointer hover:opacity-80 transition-opacity"
